...
- 최소 5턴~최대 10턴의 대화를 주고 받는다. 5턴 이후 대화 종료 버튼이 나타나도 대화를 이어갈 수 있다.
- **대화를 진행하는 과정에서 세일즈봇의 답변이 마음에 들지 않는 경우 대화를 이어가기 전에 바로 체크를 클릭한다.**
- 단, 체크를 한 번 클릭하면 수정할 수 없다. 잘못 체크한 경우, 설문지 5번 문항에 작성한다. 
...
2. 최소 5턴의 대화를 채우면 대화 종료 버튼을 확인할 수 있다. 5턴 이후 대화 종료 버튼이 나타나도 대화를 이어갈 수 있다.
...
if sst.submit is False:
    ...
    if len(sst.messages) >= NUM_TURN_LIMIT:
        btn = st.download_button(
            label="테스트 종료 및 txt 파일 저장: 대화 종료를 원하시면 버튼을 누르세요",
